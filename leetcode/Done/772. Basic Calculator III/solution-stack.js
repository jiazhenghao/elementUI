 ...
 * 使用一个符合 sign变量来存储 + - * /
 * 使用stack存储block的值
 * 如果遇到左括号（则进行循环处理，直到出现）右括号 （3 * （5 - 3））
 * 
 */
var calculate = function (s) {
    ...
    while (i < s.length) {
        ...
        if (c === '(') { // find the closed block use it to recursivly calculate the value inside
            ...
            operation(innerValue, sign, stack);
            //do the operation process
    ...
    while (stack.length !== 0) 
        result += stack.pop();//add all value in stack together since it only +/-
    return result;
};
/*
Runtime: 60 ms, faster than 97.83% 
Memory Usage: 37.9 MB, less than 40.00% 
*/
